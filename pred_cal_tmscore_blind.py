...
class CF_MSA_max():
    def __init__(self, search_dir, output_dir, pdb_name, rseed):
        command = 'colabfold_batch --amber --use-gpu-relax --num-seeds 5 --random-seed ' + str(rseed) + search_dir + output_dir
        print(command)
        os.system(command)
    ...
    def __init__(self, pdb1_name, search_dir, output_dir, rseed):
        ...
        for ran_seed in random_seed:
            ...
            for multi in (1, 2, 2, 2, 2, 2, 2):
                ...
                #### Colabfold part
                command = 'colabfold_batch --amber --use-gpu-relax --num-seeds 5 --max-seq ' + str(max_msa) + ' --max-extra-seq ' + str(ext_msa) + search_dir + output_dir + str(ran_seed) + '_max_' + str(max_msa) + '_ext_' + str(ext_msa)
                print(command)
                os.system(command)
