...
if __name__ == '__main__':
    ...
    path, ref = CL_input()
    #path should be the outermost directory of the cf-random run

    pdb_structures = sorted(glob(path + '/*/0_relaxed_*.pdb'))
    foldseek_files = sorted(glob(path + '/*/0_relaxed_*.foldseek'))

    #reference is to include structures that were generated seperate from cf-random
    if ref:
        ref_structures = sorted(glob(ref + '**/*.pdb', recursive=True))
        mol_ref_files  = sorted(glob(ref + '**/*.mol', recursive=True))
        pdb_structures = pdb_structures + ref_structures
        mol_files = mol_files + mol_ref_files
    ...
    list_CA_index_pdb1, list_CA_index_pdb2 = Match_Alpha_Carbons(pdb_structures[0], pdb_structures[3])
    print(pdb_structures[0], pdb_structures[3])
    ...
    if os.path.isfile(path.split('/')[0] + '_rmsd.npy'):
        ...
        for idx_base, pdb_base in enumerate(pdb_structures):
            bfactor.append(np.average(Bfactor(pdb_base)))
        ...
        for idx_base, pdb_base in enumerate(pdb_structures):
            ...
            for idx, pdb in enumerate(pdb_structures):
                ...
                _ = np.array([Calculate_RMSD(pdb_base, pdb, b,b) for b in batches if len(b) >= filter_rmsd_windows], dtype = 'f')
    ...
    #print(df_phenix['bits'].max())
    bits_max = df_phenix['bits'].max()
    #print(df_phenix[df_phenix.bits > float(bits_max * 0.75)])
    df_phenix = df_phenix[df_phenix.bits > float(bits_max * 0.65)]
    print("Testing depth separate")
    #print(df_phenix[df_phenix.depth.eq('full')])
    #print(df_phenix[df_phenix.depth.eq('1_2')])
    ...
    for dp in dp_list:
        ...
        if df_phenix.loc[df_phenix['depth'].eq(dp), 'RMSD PC 1'].empty:
            ...
            #x_var_ave_tmp = (df_phenix.loc[df_phenix['depth'].eq(dp), 'RMSD PC 1'])
            #y_var_ave_tmp = (df_phenix.loc[df_phenix['depth'].eq(dp), 'RMSD PC 2'])

            x_var_ave = np.average(x_var_ave_tmp); y_var_ave = np.average(y_var_ave_tmp)
            #x_var_ave = np.average(reject_outliers(x_var_ave_tmp)); y_var_ave = np.average(reject_outliers(y_var_ave_tmp))
...
#    df_phenix['target_color'] = df_phenix['target'].map(lambda x: color_dict[x])
    ...
    ##### plot the depth scatter plot

    #clus_dist_diff.astype(float)
    ...
    if np.all(clus_dist_diff == 0):
        ...
        plt.savefig(path.split('/')[0] + 'RMSD_PC_depth.png')
        plt.clf()
